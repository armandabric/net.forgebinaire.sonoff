...
interprets the volume argument) isn't a capability or Flow argument
either — it's rarely changed, so it's a device **setting** instead (device
page → advanced settings), defaulting to Liter. Changing it pushes the
new unit to the device; it doesn't convert any already-configured amount.
...
drivers/hydro-one/driver.settings.compose.json — advanced settings (device settings page)
